        ...
        {page.rows.map((row, i) => (
          <Row key={`r-${i}`} className="mt-4">
            ...
            <Col md={12} className="mt-4">
              {row.label && <p className="text-small">{row.label}</p>}
              {row.link && <Link to={row.link.link} className="link-arrow">{row.link.name} <Icon className="ml-2" variant="arrow-right" /></Link>}
            </Col>
          </Row>
        ))}
      </Container>
